...
public class ArTapSpawnerLinesBuilder : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private GameObject arObjParentPrefab;
    ...
    private void spawn()
    {
        GameObject arObjParent = Instantiate(arObjParentPrefab);
        arObjParent.transform.position = spawnPose.position;
        arObjParent.transform.rotation = spawnPose.rotation;

        GameObject spawnedObj = Instantiate(spawnObjPrefab, arObjParent.transform);
